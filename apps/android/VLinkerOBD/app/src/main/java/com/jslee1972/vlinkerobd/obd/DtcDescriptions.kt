...
/**
 * Human-readable descriptions for the SAE J2012 *generic* (manufacturer-independent) DTCs —
 * the ones whose second character is "0" and therefore mean the same thing on every vehicle.
 * This is a curated set of the most commonly seen generic codes, not an exhaustive database:
 * manufacturer-specific codes (second character "1"/"2"/"3") vary per brand/model and aren't
 * fabricated here — [describe] falls back to an honest "no built-in description" message
 * instead of guessing.
 */
object DtcDescriptions {

    private val descriptionsZh: Map<String, String> = mapOf(
    ...
    /** Chinese category label from the DTC's leading letter (P/C/B/U). */
    ...
    fun describe(code: String): String =
        descriptionsZh[code.uppercase()] ?: "此故障碼尚無內建說明，建議查詢車廠維修手冊或委由專業技師診斷。"
}
